...
void TLD::storeCurrentData()
{
    ...
    if(currBB)//Store old bounding box (if any)
    {
       ...
       // ROS_INFO_STREAM("current-x="<<prevBB->x);
       
            if(prevBB->width< MIN_WIN)
                twi.linear.x= VEL_LINE ;
            else if(prevBB->width>MAX_WIN)
                twi.linear.x=-VEL_LINE;
            if(prevBB->x+prevBB->width/2 > VIDEO_CENTER_X+OFFSET)
                twi.angular.z=-VEL_TURN;
            else if(prevBB->x+prevBB->width/2 < VIDEO_CENTER_X+OFFSET)
                twi.angular.z=VEL_TURN;
        vel_pub.publish(twi);
