...
#define VEL_LINE 0.25
#define VEL_TURN 0.25
...
namespace tld
{
    ...
    void TLD::fuseHypotheses(const Mat& colorImg)
    {
        ...
        ofstream fout;
        fout.open("/home/davidhan/catkin_ws02/src/kcf_tld_tracking_ros/data_opentld.txt");
        deleteCurrentBB();
        ...
        if (isTrackerValid)
        {
            ...
            currBB = tldCopyRect(&trackerBB);
            if(currBB->width< MIN_WIN)
                twi.linear.x= VEL_LINE ;
            else if(currBB->width>MAX_WIN)
                twi.linear.x=-VEL_LINE;
            if(currBB->x+currBB->width/2 > VIDEO_CENTER_X+OFFSET)
                twi.angular.z=-VEL_TURN;
            else if(currBB->x+currBB->width/2 < VIDEO_CENTER_X+OFFSET)
                twi.angular.z=VEL_TURN;
        vel_pub.publish(twi);
           ROS_INFO_STREAM("cmd_linear="<<twi.linear.x<<"，cmd_turn="<<twi.angular.z);
           fout<<"cmd_linear="<<twi.linear.x<<"，cmd_turn="<<twi.angular.z<<endl;
           // ROS_INFO_STREAM("widi is "<<currBB->width); 
            valid = true;
